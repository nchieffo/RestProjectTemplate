...
public class LogbackServlet extends HttpServlet {
	...
	private static final String DEFAULT_LOGGER_NAME = "it.tecla";
    ...
    private String buildHtml(List<Logger> loggers) {
        ...
        String configureLoggerForm = "<h5>Configuration</h5><div class='row'><form method='post'><div class='row'><div class=''><input class='u-full-width' id='loggerName' name='loggerName' type='text' value='" + DEFAULT_LOGGER_NAME + "'></div></div><div class='row'><div class='two columns'><select class='u-full-width' id='level' name='level'><option value='all'>all</option><option value='trace'>trace</option><option value='debug' selected='selected'>debug</option><option value='info'>info</option><option value='warn'>warn</option><option value='error'>error</option><option value='off'>off</option></select></div><div class='three columns'><input class='button-primary' value='Reconfigure logger' type='submit'></div><div class='three columns'><input name='reloadFromDisk' value='Clear/Reload from disk !' type='submit'></div></div></form></div>";
        ...
        for (Logger logger : loggers) {
            loggersTableRows += "<tr><td class='logger-name' style='cursor: pointer;' onclick='document.getElementById(\"loggerName\").value = this.innerHTML'>" + logger.getName() + "</td><td class='logger-level'>" + logger.getEffectiveLevel() + "</td></tr>" + lineSeparator;
        }
