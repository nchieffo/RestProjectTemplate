...
public class LogbackServlet extends HttpServlet {
	...
	private String buildHtml(List<Logger> loggers) {
		...
		String showLogFiles = "<h5>Log files</h5><div class='row'><form target='_blank' method='get'><div class='row'><div class='two columns'><select class='u-full-width' id='logFile' name='logFile'>"
		...
		for (Logger logger : loggers) {
			loggersTableRows += "<tr><td class='logger-name' style='cursor: pointer;' onclick='document.getElementById(\"loggerName\").value = this.innerHTML'>"
					+ logger.getName() + "</td><td class='logger-level'>" + logger.getEffectiveLevel() + "</td></tr>"
					+ lineSeparator;
		}
